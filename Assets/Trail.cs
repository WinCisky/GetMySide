...
public class Trail : MonoBehaviour {
    ...
    {
        //TO FIX
        //save previous x,z position and use that
        ...
        for (int i = 0; i < acquired_segments; i++)
            trail.SetPosition(i, new Vector3(points[i].x, (points[i].y - (i) / 1.5f), points[i].z));
        ...
        */
        ...
        if (acquired_segments < segments)
            acquired_segments++;
        for (int i = 0; i < acquired_segments; i++)
            trail.SetPosition(i, new Vector3(points[i].x, (points[i].y - (i) / 7f), points[i].z));
        Gradient gradient = new Gradient();
    }
}
