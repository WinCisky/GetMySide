    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        if (shader && material)
        {
            ...
            Vector2 pos = new Vector2(
                GameManager.GM.mainCamera.WorldToScreenPoint(BH.transform.position).x / GameManager.GM.mainCamera.pixelWidth,
                1 - GameManager.GM.mainCamera.WorldToScreenPoint(BH.transform.position).y / GameManager.GM.mainCamera.pixelHeight);
